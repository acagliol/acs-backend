...
from utils import LambdaError, invoke_lambda, authorize, db_select, create_response

# Initialize the SES client with dynamic region
...
def process_and_send_email(event):
    ...
    try:
        ...
        parsed_event = invoke_lambda(os.environ.get("PARSE_EVENT_FUNCTION_NAME", "ParseEvent"), event)
    ...
    # Rate limit check
    invoke_lambda(os.environ.get("RATE_LIMIT_AWS_FUNCTION_NAME", "RateLimitAWS"), {'client_id': account_id, 'session': session_id})
